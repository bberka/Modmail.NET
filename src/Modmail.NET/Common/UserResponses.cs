...
public static class UserResponses
{
    ...
    {
        var feedbackDone = new DiscordEmbedBuilder().WithTitle(LangKeys.FEEDBACK_RECEIVED.GetTranslation())
            .WithCustomTimestamp()
            .WithGuildInfoFooter()
    ...
    {
        var embed = new DiscordEmbedBuilder().WithTitle(LangKeys.YOUR_TICKET_HAS_BEEN_CLOSED.GetTranslation())
    ...
    public static DiscordMessageBuilder GiveFeedbackMessage(Ticket ticket, GuildOption guildOption)
    {
        ...
        {
            new DiscordButtonComponent(ButtonStyle.Primary, UtilInteraction.BuildKey("star", 1, ticket.Id), "1", false,
                new DiscordComponentEmoji("⭐")),
            new DiscordButtonComponent(ButtonStyle.Primary, UtilInteraction.BuildKey("star", 2, ticket.Id), "2", false,
                new DiscordComponentEmoji("⭐")),
            new DiscordButtonComponent(ButtonStyle.Primary, UtilInteraction.BuildKey("star", 3, ticket.Id), "3", false,
                new DiscordComponentEmoji("⭐")),
            new DiscordButtonComponent(ButtonStyle.Primary, UtilInteraction.BuildKey("star", 4, ticket.Id), "4", false,
                new DiscordComponentEmoji("⭐")),
            new DiscordButtonComponent(ButtonStyle.Primary, UtilInteraction.BuildKey("star", 5, ticket.Id), "5", false,
                new DiscordComponentEmoji("⭐"))
        };

        var ticketFeedbackEmbed = new DiscordEmbedBuilder().WithTitle(LangKeys.FEEDBACK.GetTranslation())
    ...
    {
        var embed = new DiscordEmbedBuilder().WithGuildInfoFooter(guildOption)
    ...
    {
        var embed = new DiscordEmbedBuilder().WithTitle(LangKeys.YOU_HAVE_BEEN_BLACKLISTED.GetTranslation())
    ...
    public static DiscordMessageBuilder YouHaveCreatedNewTicket(
        DiscordGuild guild,
        GuildOption option,
    ...
    {
        var embed = new DiscordEmbedBuilder().WithTitle(LangKeys.YOU_HAVE_CREATED_NEW_TICKET.GetTranslation())
        ...
        if (ticketTypes.Count > 0)
        {
            var selectBox = new DiscordSelectComponent(UtilInteraction.BuildKey("ticket_type", ticketId.ToString()),
                LangKeys.PLEASE_SELECT_A_TICKET_TYPE.GetTranslation(), ticketTypes
                    .Select(x => new DiscordSelectComponentOption(x.Name, x.Key.ToString(), x.Description, false,
                        !string.IsNullOrWhiteSpace(x.Emoji) ? new DiscordComponentEmoji(x.Emoji) : null))
                    .ToList());
    ...
    {
        var embed = new DiscordEmbedBuilder().WithTitle(LangKeys.YOU_HAVE_BEEN_REMOVED_FROM_BLACKLIST.GetTranslation())
