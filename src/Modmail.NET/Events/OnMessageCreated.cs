...
public static class OnMessageCreated
{
    ...
    internal static async Task HandlePrivateTicketMessageAsync(
    ...
    )
    {
        ...
        const string logMessage =
            $"[{nameof(OnMessageCreated)}]{nameof(HandlePrivateTicketMessageAsync)}({{ChannelId}},{{AuthorId}},{{MessageContent}})";
        try
        {
            //keeps other threads for same user locked until this one is done
            using var metran = ProcessingUserMessageContainer.AddTransaction(userId);
            if (metran is null)
            {
                //VERY UNLIKELY TO HAPPEN
                await channel.SendMessageAsync(Embeds.Error(LangData.This.GetTranslation(LangKeys.SYSTEM_IS_BUSY),
                    LangData.This.GetTranslation(LangKeys.YOUR_MESSAGE_COULD_NOT_BE_PROCESSED)));
                return;
            }
    ...
    internal static async Task HandleGuildTicketMessageAsync(
        DiscordClient sender,
        DiscordMessage message,
        DiscordChannel channel,
        DiscordUser modUser,
    ...
    {
        const string logMessage =
            $"[{nameof(OnMessageCreated)}]{nameof(HandleGuildTicketMessageAsync)}({{ChannelId}},{{AuthorId}},{{MessageContent}})";
        try
        {
            if (message.Content.StartsWith(BotConfig.This.BotPrefix))
                return;

            using var metran = ProcessingUserMessageContainer.AddTransaction(message.Author.Id);
            if (metran is null)
            {
                await channel.SendMessageAsync(Embeds.Error(LangData.This.GetTranslation(LangKeys.SYSTEM_IS_BUSY),
                    LangData.This.GetTranslation(LangKeys.YOUR_MESSAGE_COULD_NOT_BE_PROCESSED)));
