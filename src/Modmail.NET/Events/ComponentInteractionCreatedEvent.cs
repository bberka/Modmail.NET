...
public static class ComponentInteractionCreatedEvent
{
  ...
  public static async Task ComponentInteractionCreated(
    ...
    try {
      ...
      Log.Warning(
                  ex,
                  "[{Source}] BotExceptionBase: Error processing component interaction. CustomId: {CustomId}, UserId: {UserId}, ChannelId: {ChannelId}, InteractionId: {InteractionId}, MessageId: {MessageId}",
                  nameof(ComponentInteractionCreatedEvent),
                  args.Interaction?.Data?.CustomId,
                  args.User?.Id,
                  args.Channel?.Id,
