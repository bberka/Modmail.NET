...
public static class ComponentInteractionCreated
{
    ...
    public static async Task Handle(DiscordClient sender, ComponentInteractionCreateEventArgs args)
    {
        const string logMessage =
            $"[{nameof(ComponentInteractionCreated)}]{nameof(Handle)}({{CustomId}},{{UserId}},{{ChannelId}},{{InteractionId}},{{MessageId}})";
        try
        {
            ...
            switch (interactionName)
            {
                case "star":
                {
                    ...
                    Log.Information(logMessage, args.Interaction?.Data?.CustomId, args.User?.Id, args.Channel?.Id, args.Interaction?.Id,
                        args.Message?.Id);
                    break;
                }
                case "ticket_type":
        ...
        {
            Log.Warning(ex, logMessage, args.Interaction?.Data?.CustomId, args.User?.Id, args.Channel?.Id, args.Interaction?.Id, args.Message?.Id);
        }
        catch (Exception ex)
        {
            Log.Fatal(ex, logMessage, args.Interaction?.Data?.CustomId, args.User?.Id, args.Channel?.Id, args.Interaction?.Id, args.Message?.Id);
        }
    }
}