...
{
    private readonly Dictionary<string, IReadOnlyDictionary<Lang, string>> _languages;
    ...
    public LangProvider(IOptions<BotConfig> options)
    {
        ...
        if (!Directory.Exists(langDir))
            throw new DirectoryNotFoundException(
                "Language data directory not found! Please make sure you have the correct directory structure. Expected directory : " + langDir);

        var files = Directory.GetFiles(langDir, "*.json");
        if (files.Length == 0)
            throw new FileNotFoundException(
                "No language files found in the directory! Please make sure you have the correct directory structure. Expected directory : " +
                langDir);
        ...
        foreach (var file in files)
        {
            ...
            {
                var newValue = new StringBuilder(kp.Value).Replace("{NewLine}", Environment.NewLine)
                    .Replace("{Tab}", "\t")
    ...
    {
        return ServiceLocator.GetBotConfig()
            .DefaultLanguage;
    }

    public string Translate(Lang key, params string[] args)
    {
        ...
        var newArgs = args.Select(arg => Enum.TryParse<Lang>(arg, out var newArg) ? Translate(newArg) : arg)
            .ToArray();
