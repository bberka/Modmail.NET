...
public class GetLatestMetricHandler : IRequestHandler<GetLatestMetricQuery, MetricDto?>
{
    ...
    {
        var data = await _dbContext.Statistics.AsNoTracking()
            .OrderByDescending(x => x.RegisterDateUtc)
            .FirstOrDefaultAsync(cancellationToken);
        if (data is null) return null;

        var totalTickets = await _dbContext.Tickets.AsNoTracking()
        ...
        var openTicketsChartDataArray = await _dbContext.Tickets.FilterActive()
        ...
        var closedTicketsChartDataArray = await _dbContext.Tickets.FilterClosed()
        ...
        var userMessageCountChartDataArray = await _dbContext.Messages.Where(x => !x.SentByMod)
            .GroupBy(x => x.RegisterDateUtc.Date)
            .OrderByDescending(x => x.Key)
            .Take(metricsTakeDay)
            .Select(x => new ChartItemDto<DateTime, int>(x.Key, x.Count()))
        ...
        var modMessageCountChartDataArray = await _dbContext.Messages.Where(x => x.SentByMod)
            .GroupBy(x => x.RegisterDateUtc.Date)
            .OrderByDescending(x => x.Key)
            .Take(metricsTakeDay)
            .Select(x => new ChartItemDto<DateTime, int>(x.Key, x.Count()))
        ...
        var ticketTypeChartDataArray = await _dbContext.Tickets.GroupBy(x => x.TicketTypeId)
            .Select(group =>
                new ChartItemDto<string, int>(group.Key == null ? Lang.NoType.Translate() : group.FirstOrDefault()!.TicketType!.Name, group.Count()))
            .ToArrayAsync(cancellationToken);


        var ticketPriorityChartDataArray = await _dbContext.Tickets.GroupBy(x => x.Priority)
            .Select(group => new ChartItemDto<string, int>(group.Key.ToString(), group.Count()))
            .ToArrayAsync(cancellationToken);
