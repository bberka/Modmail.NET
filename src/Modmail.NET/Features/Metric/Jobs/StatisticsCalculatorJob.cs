...
public class StatisticsCalculatorJob : HangfireRecurringJobBase
{
    ...
    public override async Task Execute()
    {
        ...
        if (statDays is < MetricConstants.StatisticsCalculateDaysMin or > MetricConstants.StatisticsCalculateDaysMax)
            statDays = MetricConstants.DefaultStatisticsCalculateDays;

        var statDate = UtilDate.GetNow()
            .AddDays(-statDays);
    ...
    private static async Task<double?> GetAvgResponseTimeSeconds(ModmailDbContext dbContext, DateTime statDate)
    {
        try
        {
            return await dbContext.Messages.FilterByRegisterDateStart(statDate)
                .Select(x => new
                {
                    x.TicketId,
                    x.RegisterDateUtc,
                ...
                {
                    TicketId = group.Key,
                    FirstUserMessageTime = group.Where(x => !x.SentByMod)
                        .OrderBy(x => x.RegisterDateUtc)
                        .Select(x => x.RegisterDateUtc)
                        .DefaultIfEmpty()
                        .FirstOrDefault(),
                    FirstModResponseTime = group.Where(x => x.SentByMod)
                        .OrderBy(x => x.RegisterDateUtc)
                        .Select(x => x.RegisterDateUtc)
                        .DefaultIfEmpty()
                        .FirstOrDefault()
    ...
    private static async Task<double?> GetAvgTicketsClosedPerDay(ModmailDbContext dbContext, DateTime statDate)
    {
        try
        {
            return await dbContext.Tickets.FilterByRegisterDateStart(statDate)
