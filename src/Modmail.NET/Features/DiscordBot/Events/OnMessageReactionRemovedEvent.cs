...
public static class OnMessageReactionRemovedEvent
{
    ...
    public static async Task OnMessageReactionRemoved(DiscordClient client, MessageReactionRemovedEventArgs args)
    {
        if (args.User.IsBot)
        {
            Log.Debug("[{Source}] Ignoring reaction removed by bot. UserId: {UserId}", nameof(OnMessageReactionRemovedEvent), args.User.Id);
            return;
        }

        if (args.Emoji.Name == TicketConstants.ProcessedReactionDiscordEmojiUnicode)
        {
            Log.Debug("[{Source}] Ignoring processed reaction emoji removed. EmojiName: {EmojiName}", nameof(OnMessageReactionRemovedEvent),
                args.Emoji.Name);
            return;
        }
    ...
    {
        Log.Debug("[{Source}] Processing reaction removed in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}, Emoji: {Emoji}",
            nameof(OnMessageReactionRemovedEvent), args.Channel.Id, args.Message.Id, args.Emoji.Name);
        ...
        if (ticket is null)
        {
            Log.Warning(
                ...
                nameof(OnMessageReactionRemovedEvent), args.Channel.Id, args.Message.Id, messageEntity.TicketId);
            return;
        }

        await RemoveReactionFromMirroredMessage(client, ticket.ModMessageChannelId, messageEntity.MessageDiscordId, args.Emoji);
    }
    ...
    {
        Log.Debug("[{Source}] Processing reaction removed in ticket channel. ChannelId: {ChannelId}, MessageId: {MessageId}, Emoji: {Emoji}",
            nameof(OnMessageReactionRemovedEvent), args.Channel.Id, args.Message.Id, args.Emoji.Name);
        ...
        if (ticketId == Guid.Empty)
        {
            Log.Warning("[{Source}] Could not extract valid TicketId from channel topic. ChannelId: {ChannelId}, Topic: {Topic}",
                nameof(OnMessageReactionRemovedEvent), args.Channel.Id, topic);
            return;
        }
        ...
        if (messageEntity is null)
        {
            Log.Debug(
                ...
                nameof(OnMessageReactionRemovedEvent), args.Channel.Id, args.Message.Id, ticketId);
            return;
        }
        ...
        await RemoveReactionFromMirroredMessage(client, ticket.PrivateMessageChannelId, messageEntity.MessageDiscordId, args.Emoji);
    }
    ...
    )
    {
        if (!channelId.HasValue || !messageId.HasValue)
        {
            Log.Warning(
                ...
                nameof(OnMessageReactionRemovedEvent), channelId, messageId, emoji.Name);
            return;
        }
        ...
        try
        {
            ...
            Log.Information("[{Source}] Processed reaction removed {ChannelId} " + "{MessageId} {MessageAuthor} {Emoji}",
                nameof(OnMessageReactionRemovedEvent), channel.Id, message.Id, message.Author?.Id, emoji);
        }
