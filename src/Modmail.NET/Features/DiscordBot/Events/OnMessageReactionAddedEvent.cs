...
public class OnMessageReactionAddedEvent
{
    ...
    public static async Task OnMessageReactionAdded(DiscordClient client, MessageReactionAddedEventArgs args)
    {
        if (args.User.IsBot)
        {
            Log.Debug("[{Source}] Ignoring reaction added by bot. UserId: {UserId}", nameof(OnMessageReactionAddedEvent), args.User.Id);
            return;
        }

        if (args.Emoji.Name == TicketConstants.ProcessedReactionDiscordEmojiUnicode)
        {
            Log.Debug("[{Source}] Ignoring processed reaction emoji. EmojiName: {EmojiName}", nameof(OnMessageReactionAddedEvent), args.Emoji.Name);
            return;
        }
    ...
    {
        Log.Debug("[{Source}] Processing reaction added in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}, Emoji: {Emoji}",
            nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id, args.Emoji.Name);
        ...
        if (args.Message.Author?.Id != client.CurrentUser.Id)
        {
            Log.Debug(
                ...
                nameof(OnMessageReactionRemovedEvent), args.Channel.Id, args.Message.Id);
            return;
        }
        ...
        {
            Log.Debug("[{Source}] Ignoring bots reaction in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id);
            return;
        }

        var dbContext = scope.ServiceProvider.GetRequiredService<ModmailDbContext>();
        var messageEntity = await dbContext.Messages.FirstOrDefaultAsync(x => x.SentByMod && x.BotMessageId == args.Message.Id);
        if (messageEntity is null)
        {
            Log.Debug("[{Source}] No matching TicketMessage found for reaction in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id);
            return;
        }
        ...
        if (ticket is null)
        {
            Log.Warning(
                ...
                nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id, messageEntity.TicketId);
            return;
        }

        await AddReactionToMirroredMessage(client, ticket.ModMessageChannelId, messageEntity.MessageDiscordId, args.Emoji);
    }
    ...
    {
        Log.Debug("[{Source}] Processing reaction added in ticket channel. ChannelId: {ChannelId}, MessageId: {MessageId}, Emoji: {Emoji}",
            nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id, args.Emoji.Name);
        ...
        if (ticketId == Guid.Empty)
        {
            Log.Warning("[{Source}] Could not extract valid TicketId from channel topic. ChannelId: {ChannelId}, Topic: {Topic}",
                nameof(OnMessageReactionAddedEvent), args.Channel.Id, topic);
            return;
        }
        ...
        if (messageEntity is null)
        {
            Log.Debug(
                ...
                nameof(OnMessageReactionAddedEvent), args.Channel.Id, args.Message.Id, ticketId);
            return;
        }
        ...
        await AddReactionToMirroredMessage(client, ticket.PrivateMessageChannelId, messageEntity.MessageDiscordId, args.Emoji);
    }
    ...
    )
    {
        if (!channelId.HasValue || !messageId.HasValue)
        {
            Log.Warning(
                ...
                nameof(OnMessageReactionAddedEvent), channelId, messageId, emoji.Name);
            return;
        }
        ...
        try
        {
            ...
            Log.Information("[{Source}] Processed reaction added {ChannelId} {MessageId} " + "{MessageAuthor} {Emoji}",
                nameof(OnMessageReactionAddedEvent), channel.Id, message.Id, message.Author?.Id, emoji.Name);
        }
        catch (NotFoundException)
        {
            Log.Warning("[{Source}] Mirrored message not found, cannot add reaction. ChannelId: {ChannelId}, MessageId: {MessageId}, Emoji: {Emoji}",
                nameof(OnMessageReactionAddedEvent), channelId, messageId, emoji.Name);
        }
        catch (Exception ex)
        {
