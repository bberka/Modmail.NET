...
public static class OnMessageUpdatedEvent
{
    ...
    public static async Task OnMessageUpdated(DiscordClient client, MessageUpdatedEventArgs args)
    {
        ...
        {
            Log.Debug("[{Source}] Ignoring message update from bot. UserId: {UserId}, MessageId: {MessageId}", nameof(OnMessageUpdatedEvent),
                args.Author.Id, args.Message.Id);
            return;
        }

        if (args.MessageBefore?.Content == args.Message.Content)
        {
            Log.Debug("[{Source}] Message content unchanged, ignoring update. MessageId: {MessageId}", nameof(OnMessageUpdatedEvent),
                args.Message.Id);
            return;
        }
    ...
    {
        Log.Debug("[{Source}] Processing message update in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
            nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id);

        if (args.Message.Author?.Id == client.CurrentUser.Id)
        {
            Log.Debug("[{Source}] Ignoring message update on bot's own message in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id);
            return;
        }
        ...
        var messageEntity = await dbContext.Messages.FirstOrDefaultAsync(x =>
            !x.SentByMod && x.MessageDiscordId == args.Message.Id && args.Message.Author != null && x.SenderUserId == args.Message.Author.Id);
        if (messageEntity is null)
        {
            Log.Debug("[{Source}] No matching TicketMessage found for update in private channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id);
            return;
        }

        var ticket = await dbContext.Tickets.FilterActive()
        ...
        if (ticket is null)
        {
            Log.Warning(
                ...
                nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id, messageEntity.TicketId);
            return;
        }

        await UpdateMirroredMessage(client, messageEntity, dbContext, scope, ticket.ModMessageChannelId, messageEntity.BotMessageId, args.Message,
            ticket.Anonymous);
    }
    ...
    {
        Log.Debug("[{Source}] Processing message update in ticket channel. ChannelId: {ChannelId}, MessageId: {MessageId}",
            nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id);
        ...
        {
            Log.Warning("[{Source}] Could not extract valid TicketId from channel topic. ChannelId: {ChannelId}, Topic: {Topic}",
                nameof(OnMessageUpdatedEvent), args.Channel.Id, topic);
            return;
        }
        ...
        if (messageEntity is null)
        {
            Log.Debug(
                ...
                nameof(OnMessageUpdatedEvent), args.Channel.Id, args.Message.Id, ticketId);
            return;
        }

        var ticket = await dbContext.Tickets.FilterById(ticketId)
            .FilterActive()
        ...
        await UpdateMirroredMessage(client, messageEntity, dbContext, scope, ticket.PrivateMessageChannelId, messageEntity.BotMessageId, args.Message,
            ticket.Anonymous);
    }
    ...
    )
    {
        try
        {
            ...
            var embed = updatedMessage.Channel!.IsPrivate
            ...
            Log.Information("[{Source}] Processed message edited {ChannelId} " + "{MessageId} {MessageAuthor}", nameof(OnMessageUpdatedEvent),
                channel.Id, message.Id, updatedMessage.Author?.Id);
        }
        catch (NotFoundException)
        {
            Log.Warning("[{Source}] Mirrored message not found, cannot update. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageUpdatedEvent), channelId, messageId);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[{Source}] An error occurred while updating the mirrored message. ChannelId: {ChannelId}, MessageId: {MessageId}",
                nameof(OnMessageUpdatedEvent), channelId, messageId);
        }
    }


    private static DiscordEmbedBuilder MessageEditedUpdatePrivateMessage(
