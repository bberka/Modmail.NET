...
public static class ModalSubmittedEvent
{
    ...
    {
        Log.Debug("[{Source}] Modal submitted. CustomId: {CustomId}, InteractionId: {InteractionId}", nameof(ModalSubmittedEvent),
            args.Interaction.Data.CustomId, args.Interaction.Id);
        ...
        try
        {
            ...
            var (interactionName, parameters) = UtilInteraction.ParseKey(args.Interaction.Data.CustomId);
            ...
            switch (interactionName)
            {
                case "feedback":
                    ...
                    await args.Interaction.CreateResponseAsync(DiscordInteractionResponseType.ChannelMessageWithSource,
                        new DiscordInteractionResponseBuilder().AsEphemeral()
                            .WithContent(Lang.ThankYouForFeedback.Translate()));
                    break;
                case "close_ticket_with_reason":
                    ...
                    break;
                default:
                    Log.Warning("[{Source}] Unknown interaction name: {InteractionName}, CustomId: {CustomId}", nameof(ModalSubmittedEvent),
                        interactionName, args.Interaction.Data.CustomId);
                    break;
            }
        }
        catch (ModmailBotException ex)
        {
            Log.Warning(ex, "[{Source}] ModmailBotException: Error processing modal submission. CustomId: {CustomId}, InteractionId: {InteractionId}",
                nameof(ModalSubmittedEvent), args.Interaction.Data.CustomId, args.Interaction.Id);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[{Source}] Unhandled exception processing modal submission. CustomId: {CustomId}, InteractionId: {InteractionId}",
                nameof(ModalSubmittedEvent), args.Interaction.Data.CustomId, args.Interaction.Id);
        }
    }
    ...
    )
    {
        ...
        Log.Information("[{Source}] Feedback processed. TicketId: {TicketId}, StarCount: {StarCount}, InteractionId: {InteractionId}",
            nameof(ModalSubmittedEvent), ticketId, starCount, args.Interaction.Id);
    }
    ...
    )
    {
        ...
        await sender.Send(new ProcessCloseTicketCommand(args.Interaction.User.Id, ticketId, textInput));

        Log.Information("[{Source}] Ticket closed with reason. TicketId: {TicketId}, InteractionId: {InteractionId}", nameof(ModalSubmittedEvent),
            ticketId, args.Interaction.Id);
    }
}