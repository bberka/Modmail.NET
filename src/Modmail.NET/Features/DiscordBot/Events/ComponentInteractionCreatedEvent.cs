...
public static class ComponentInteractionCreatedEvent
{
    ...
    public static async Task ComponentInteractionCreated(DiscordClient client, ComponentInteractionCreatedEventArgs args)
    {
        Log.Debug(
            ...
            nameof(ComponentInteractionCreatedEvent), args.Interaction.Data.CustomId, args.User.Id, args.Channel.Id, args.Interaction.Id,
            args.Message.Id);
        ...
        try
        {
            ...
            switch (interactionName)
            {
                ...
                case "close_ticket_with_reason":
                    ...
                    break;
                default:
                    Log.Warning("[{Source}] Unknown interaction name: {InteractionName}, CustomId: {CustomId}",
                        nameof(ComponentInteractionCreatedEvent), interactionName, args.Interaction.Data.CustomId);
                    break;
            }
        }
    ...
    private static async Task ProcessStarInteraction(ComponentInteractionCreatedEventArgs args, ulong messageId)
    {
        var key = args.Interaction.Data.CustomId;
        try
        {
            ...
            Log.Information("[{Source}] Star interaction processed. TicketId: {TicketId}, StarCount: {StarCount}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), ticketId, starCount, args.Interaction?.Id);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[{Source}] Error processing star interaction submission. CustomId: {CustomId}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), args.Interaction.Data.CustomId, args.Interaction?.Id);
        }
        ...
        {
            var modal = new DiscordInteractionResponseBuilder().WithTitle(Lang.Feedback.Translate())
                .WithCustomId(UtilInteraction.BuildKey("feedback", starCount, ticketId, messageId))
                .AddComponents(new DiscordTextInputComponent(Lang.Feedback.Translate(), "feedback", Lang.PleaseTellUsReasonsForYourRating.Translate(),
                    style: DiscordTextInputStyle.Paragraph, required: false, max_length: DbLength.Message));
            return modal;
        }
    }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                Log.Warning("[{Source}] No ticket type selected. InteractionId: {InteractionId}, MessageId: {MessageId}",
                    nameof(ComponentInteractionCreatedEvent), args.Interaction?.Id, messageId);
                return;
            }

            await sender.Send(new ProcessChangeTicketTypeCommand(args.User.Id, ticketId, selectedTypeKey));

            Log.Information(
                "[{Source}] Ticket type changed. TicketId: {TicketId}, SelectedTypeKey: {SelectedTypeKey}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), ticketId, selectedTypeKey, args.Interaction?.Id);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[{Source}] Error processing process ticket type interaction. CustomId: {CustomId}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), args.Interaction.Data.CustomId, args.Interaction?.Id);
        }
    }
    ...
    private static async Task ProcessCloseTicketInteraction(ComponentInteractionCreatedEventArgs args)
    {
        ...
        try
        {
            ...
            Log.Information("[{Source}] Close ticket interaction triggered. TicketId: {TicketId}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), ticketId, args.Interaction?.Id);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[{Source}] Error processing  process close ticket interaction. CustomId: {CustomId}, InteractionId: {InteractionId}",
                nameof(ComponentInteractionCreatedEvent), args.Interaction.Data.CustomId, args.Interaction?.Id);
        }
        ...
        {
            var modal = new DiscordInteractionResponseBuilder().WithTitle(Lang.CloseTicket.Translate())
                .WithCustomId(UtilInteraction.BuildKey("close_ticket_with_reason", ticketId))
                .AddComponents(new DiscordTextInputComponent(Lang.Reason.Translate(), "reason", Lang.EnterReasonForClosingThisTicket.Translate(),
                    style: DiscordTextInputStyle.Paragraph, required: false, max_length: DbLength.Reason));
            return modal;
        }
    }
}