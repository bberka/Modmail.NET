...
public class TicketSlashCommands
{
    ...
    public async Task CloseTicket(
        ...
        [Parameter("reason")] [Description("Ticket closing reason. User will be notified of this reason.")] string? reason = null
    )
    {
        ...
        try
        {
            ...
            if (ticket.OpenerUserId != ctx.User.Id)
            {
                ...
                if (!isAnyTeamMember)
                {
                    await ctx.Interaction.CreateResponseAsync(DiscordInteractionResponseType.UpdateMessage, ModmailInteractions
                        .Error(Lang.YouDoNotHavePermissionToUseThisCommand.Translate())
                        .AsEphemeral());
                    return;
                }
            }
    ...
    public async Task SetPriority(SlashCommandContext ctx, [Parameter("priority")] [Description("Priority of the ticket")] TicketPriority priority)
    {
    ...
    public async Task AddNote(SlashCommandContext ctx, [Parameter("note")] [Description("Note to add")] string note)
    {
    ...
    public async Task SetType(
        SlashCommandContext ctx,
        [Parameter("type")] [Description("Type of the ticket")] [SlashAutoCompleteProvider(typeof(TicketTypeProvider))] string type
    )
    {
