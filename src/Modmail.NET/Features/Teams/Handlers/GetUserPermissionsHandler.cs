...
public class GetUserPermissionsHandler : IRequestHandler<GetUserPermissionsQuery, AuthPolicy[]>
{
    ...
    public GetUserPermissionsHandler(IDbContextFactory<ModmailDbContext> factory, IOptions<BotConfig> options)
    {
        _factory = factory;
        _options = options;
    }
    ...
    public async ValueTask<AuthPolicy[]> Handle(GetUserPermissionsQuery request, CancellationToken cancellationToken)
    {
        ...
        var permissions = await dbContext.TeamPermissions.AsNoTracking()
            .Include(x => x.Team)
        ...
        if (permissions.Contains(AuthPolicy.SuperUser))
            return [AuthPolicy.SuperUser]; //TODO: implement this logic also to update permission logic if allow all provided then 
