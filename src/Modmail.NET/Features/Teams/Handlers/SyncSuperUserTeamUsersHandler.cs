...
public class SyncSuperUserTeamUsersHandler : IRequestHandler<SyncSuperUserTeamUsersCommand>
{
  ...
  public SyncSuperUserTeamUsersHandler(ModmailDbContext dbContext,
                                       IOptions<BotConfig> options) {
    _dbContext = dbContext;
    _options = options;
  }

  public async Task Handle(SyncSuperUserTeamUsersCommand request, CancellationToken cancellationToken) {
  ...
  private async Task<Team> GetOrCreateSuperUserTeam(CancellationToken cancellationToken) {
    ...
    Log.Information(
                    "[{Source}] Created SuperUserTeam. TeamId: {TeamId}",
                    nameof(SyncSuperUserTeamUsersHandler),
                    team.Id
                   );
    return team;
  }
  ...
  private static void SyncTeamPermissions(Team team) {
    if (team.Permissions.Count != 1 ||
        team.Permissions.All(x => x.AuthPolicy != AuthPolicy.SuperUser)) {
      ...
      Log.Warning(
                  "[{Source}] SuperUserTeam had invalid permissions, corrected. TeamId: {TeamId}",
                  nameof(SyncSuperUserTeamUsersHandler),
                  team.Id
                 );
    }
  ...
  private async Task SyncTeamMembers(Team team, CancellationToken cancellationToken) {
    ...
    var usersToAdd = configSuperUsers.Except(team.Users.Select(x => x.UserId)).ToList();
    if (usersToAdd.Count != 0) {
      ...
      Log.Information(
                      "[{Source}] Added missing super users to SuperUserTeam. TeamId: {TeamId}, UserIds: {UserIds}",
                      nameof(SyncSuperUserTeamUsersHandler),
                      team.Id,
                      string.Join(", ", usersToAdd)
                     );
    }

    // Remove extra super users if they aren't in config
    var usersToRemove = team.Users.Select(x => x.UserId).Except(configSuperUsers).ToList();
    if (usersToRemove.Count != 0) {
      var teamUsersToRemove = team.Users.Where(x => usersToRemove.Contains(x.UserId)).ToList();
      _dbContext.RemoveRange(teamUsersToRemove);
      Log.Information(
                      "[{Source}] Removed extra super users from SuperUserTeam. TeamId: {TeamId}, UserIds: {UserIds}",
                      nameof(SyncSuperUserTeamUsersHandler),
                      team.Id,
                      string.Join(", ", usersToRemove)
                     );
    }
    ...
    if (affected > 0)
      Log.Information(
                      "[{Source}] Updated SuperUserTeam. TeamId: {TeamId}, Changes: {Changes}",
                      nameof(SyncSuperUserTeamUsersHandler),
                      team.Id,
                      affected
                     );
      ...
      Log.Information(
                      "[{Source}] SuperUserTeam is synced. TeamId: {TeamId}",
                      nameof(SyncSuperUserTeamUsersHandler),
                      team.Id
                     );
  }

  private async Task EnsureUsersAreOnlyInThisTeam(Team targetTeam, List<ulong> userIds, CancellationToken cancellationToken) {
    // Check if the accounts from config have access or are used to some team.

    var teamsWithTheseUsers = await _dbContext.Teams
                                              .Include(x => x.Users)
    ...
    if (teamsWithTheseUsers.Any())
      ...
      foreach (var t in teamsWithTheseUsers) {
        var usersToRemove = t.Users.Where(x => userIds.Contains(x.UserId)).ToList();
        _dbContext.RemoveRange(usersToRemove);
        Log.Information(
                        "[{Source}] Removed super users from other teams. TeamId: {TeamId}, UserIds: {UserIds}",
                        nameof(SyncSuperUserTeamUsersHandler),
                        t.Id,
                        string.Join(", ", usersToRemove.Select(x => x.UserId))
                       );
      }
