...
public class ProcessAddPermissionToTeamHandler : IRequestHandler<ProcessAddPermissionToTeamCommand, TeamPermission[]>
{
    ...
    public ProcessAddPermissionToTeamHandler(
    ...
    {
        var team = await _dbContext.Teams.Where(x => x.Id == request.TeamId)
        ...
        var cleanedRequestedPermissions = GetCleanedRequestedPermissions(request.Policies, permissionsUserCanAssign, team.Permissions
    ...
    private async Task<AuthPolicy[]> GetPermissionsUserCanAssign(ulong userId, CancellationToken cancellationToken)
    {
        ...
        if (isSuperUser)
            return AuthPolicy.List.Where(x => x != AuthPolicy.SuperUser)
                .ToArray();

        return userPermissions;
    }

    private static AuthPolicy[] GetCleanedRequestedPermissions(
        AuthPolicy[] permissionsRequestedToAdd,
        AuthPolicy[] permissionsUserCanAssign,
        AuthPolicy[] permissionsAlreadyAssignedToTeam
    )
    {
        var hasSuperUser = permissionsRequestedToAdd.Any(x => x == AuthPolicy.SuperUser);
        if (hasSuperUser) return [AuthPolicy.SuperUser];

        var exceptAlreadyAssigned = permissionsRequestedToAdd.Except(permissionsAlreadyAssignedToTeam)
            .ToArray();
        var onlyUserCanAssign = exceptAlreadyAssigned.Intersect(permissionsUserCanAssign)
            .ToArray();
