...
public class ProcessRemoveTeamPermissionHandler : IRequestHandler<ProcessRemoveTeamPermissionCommand, TeamPermission>
{
  ...
  public async Task<TeamPermission> Handle(ProcessRemoveTeamPermissionCommand request, CancellationToken cancellationToken) {
    var entity = await _dbContext.TeamPermissions
                                 .Include(x => x.Team)
