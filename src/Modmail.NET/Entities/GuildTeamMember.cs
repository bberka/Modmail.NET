...
public sealed class GuildTeamMember
{
    ...
    public static async Task<TeamPermissionLevel?> GetPermissionLevelAsync(ulong userId, List<ulong> roleIdList)
    {
        ...
        var teamMember = await dbContext.GuildTeamMembers.Include(x => x.GuildTeam)
            .Where(x => (x.Type == TeamMemberDataType.RoleId && roleIdList.Contains(x.Key)) ||
                        (x.Key == userId && x.Type == TeamMemberDataType.UserId))
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        return await dbContext.GuildTeamMembers.Include(x => x.GuildTeam)
