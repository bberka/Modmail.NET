...
public sealed class TicketBlacklist
{
    ...
    public static async Task ProcessAddUserToBlacklist(
        ulong userId,
        string? reason = null,
        ulong modId = 0
    )
    {
        ...
        if (activeTicket is not null)
            await activeTicket.ProcessCloseTicketAsync(userId, LangData.This.GetTranslation(LangKeys.TICKET_CLOSED_DUE_TO_BLACKLIST),
