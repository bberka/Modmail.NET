...
public sealed class GuildOption
{
    ...
    public static async Task<ulong> GetLogChannelIdAsync(ulong guildId)
    {
        ...
        return await dbContext.GuildOptions.Where(x => x.GuildId == guildId)
    ...
    public async Task<DiscordChannel> ProcessCreateLogChannel(DiscordGuild guild)
    {
        ...
        var logChannel = await guild.CreateTextChannelAsync(Const.LOG_CHANNEL_NAME, category,
            LangData.This.GetTranslation(LangKeys.MODMAIL_LOG_CHANNEL_TOPIC), permissionOverwrites);
