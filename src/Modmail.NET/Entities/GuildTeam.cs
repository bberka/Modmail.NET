...
public sealed class GuildTeam
{
    ...
    public static async Task<List<GuildTeam>> GetAllAsync()
    {
        ...
        var result = await dbContext.GuildTeams.Include(x => x.GuildTeamMembers)
            .ToListAsync();
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        var result = await dbContext.GuildTeams.FirstOrDefaultAsync(x => x.Name == name);
        if (result is null) throw new NotFoundWithException(LangKeys.TEAM, name);
        return result;
    }

    public static async Task ProcessCreateTeamAsync(
        string teamName,
    ...
    {
        var dbContext = ServiceLocator.Get<ModmailDbContext>();
        var memberEntity = await dbContext.GuildTeamMembers.FirstOrDefaultAsync(x => x.Key == teamMemberKey && x.Type == type);
    ...
    public async Task ProcessUpdateTeamAsync(
        string teamName,
        TeamPermissionLevel? permissionLevel,
    ...
    )
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                var logChannel = await ModmailBot.This.GetLogChannelAsync();
                await logChannel.SendMessageAsync(LogResponses.TeamUpdated(oldPermissionLevel, oldPingOnNewTicket, oldPingOnNewMessage, oldIsEnabled,
                    team.PermissionLevel, team.PingOnNewTicket, team.PingOnNewMessage, team.IsEnabled, team.Name));
            }
        });
    }
}