...
using DbLength = Modmail.NET.Common.Static.DbLength;
...
public sealed class Ticket
{
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        var ticket = await dbContext.Tickets.FirstOrDefaultAsync(x => x.OpenerUserId == userId && !x.ClosedDateUtc.HasValue);
    ...
    public async Task ProcessCloseTicketAsync(
    ...
    public async Task ProcessChangePriorityAsync(
        ulong modUserId,
    ...
    public async Task ProcessUserSentMessageAsync(DiscordMessage message, DiscordChannel? privateChannel = null)
    {
        ArgumentNullException.ThrowIfNull(OpenerUser);
    ...
    public static async Task ProcessCreateNewTicketAsync(
        DiscordUser user,
    ...
    )
    {
        ...
        var mailChannel =
            await guild.CreateTextChannelAsync(channelName, category, UtilChannelTopic.BuildChannelTopic(ticketId), permissionOverwrites);
        ...
        var ticketCreatedMessage = await privateChannel.SendMessageAsync(UserResponses.YouHaveCreatedNewTicket(guild,
            guildOption, ticketTypes, ticketId));
    ...
    public async Task ProcessModSendMessageAsync(
        DiscordUser modUser,
        DiscordMessage message,
        ...
        _ = Task.Run(async () =>
        {
            ...
            if (guildOption.IsEnableDiscordChannelLogging)
                if (guildOption.IsSensitiveLogging)
                {
                    ...
                    var logChannel = await ModmailBot.This.GetLogChannelAsync();
                    var embed3 = LogResponses.MessageSentByMod(message, Id, Anonymous);
                    await logChannel.SendMessageAsync(embed3);
                }
        });
    }

    public async Task ProcessAddFeedbackAsync(
        int starCount,
        string textInput,
    ...
    public async Task ProcessChangeTicketTypeAsync(
        string type,
        DiscordChannel? ticketChannel = null,
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        var tickets = await dbContext.Tickets.Where(x => !x.ClosedDateUtc.HasValue)
            .ToListAsync();
        return tickets;
    }
    ...
    public static async Task<List<Ticket>> GetTimeoutTicketsAsync(long timeoutHours)
    {
        ...
        var tickets = await dbContext.Tickets.Where(x => !x.ClosedDateUtc.HasValue && x.LastMessageDateUtc < timeoutDate)
            .ToListAsync();
        return tickets;
    }
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        var tickets = await dbContext.Tickets.Where(x => x.TicketTypeId == ticketType.Id)
            .ToListAsync();
        return tickets;
    }
    ...
    {
        await using var dbContext = ServiceLocator.Get<ModmailDbContext>();
        return await dbContext.Tickets.AnyAsync(x => x.TicketTypeId == ticketType.Id && !x.ClosedDateUtc.HasValue);
    }
